...
### RSP-Con parameters
The following parameters are only available for the RSP-Con variant. It is required to set either one of the values. If a fixed cost is wanted, setting `-max-cost` is sufficient, since the optimizer will automatically maximize the connectivity values by taking as many planning units as possible, i.e., maximize the cost.
